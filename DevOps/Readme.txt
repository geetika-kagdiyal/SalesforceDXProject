...
2. run Git command to find difference between current commit and last successfull commit and save it to list file -
 "powershell -command "git diff-tree --no-commit-id --name-only -r head^ head > list.txt""
...
5. run groovy script using below command to create the destructiveChanges xml file  -
	"groovy DestructiveXMLGenerator.groovy deltaDestruction/force-app/main/default deltaDestruction/force-app/main/default/manifest/destructiveChanges.xml"

6. run sfdx commands below to deploy on salesforce server - 
	"sfdx force:mdapi:deploy -d delta/force-app/main/default -w 30"
	
7. run sfdx commands below to deploy destructive changes on salesforce server - 
	"sfdx force:mdapi:deploy -d deltaDestruction/force-app/main/default/manifest -w 30"
